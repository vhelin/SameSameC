...
int main(int argc, char *argv[]) {
  ...
  if (argc < 5 || parse_flags_result == FAILED || g_backend == BACKEND_NONE) {
    ...
    printf("Options:\n");
    printf("-b <BANK>  ROM Bank\n");
    printf("-s <SLOT>  ROM Slot\n");
    printf("-rb <BANK> RAM Bank (for global variables)\n");
...
int parse_flags(char **flags, int flagc, int *print_usage) {
  ...
  for (count = 1; count < flagc; count++) {
    ...
    else if (!strcmp(flags[count], "-rb")) {
      if (count + 1 < flagc) {
        /* get arg */
        g_ram_bank = atoi(flags[count+1]);
      }
      else
        return FAILED;
    ...
    else if (!strcmp(flags[count], "-rs")) {
      if (count + 1 < flagc) {
        /* get arg */
        g_ram_slot = atoi(flags[count+1]);
      }
      else
        return FAILED;
